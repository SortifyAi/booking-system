...
interface ResourceImageUploadControlProps {
  resource: {
    id: string;
    name: string;
  ...
  onUpdated?: (resource: unknown) => void;
}

export function ResourceImageUploadControl({
  resource,
  onUpdated,
}: ResourceImageUploadControlProps) {
  const inputRef = useRef<HTMLInputElement | null>(null);
  const [uploading, setUploading] = useState(false);

  const organizationId = resource.organization_id ?? resource.organizationId;
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      setUploading(true);
      if (isMockMode()) {
        onUpdated?.({ ...resource, image_url: URL.createObjectURL(file) });
        toast.success('Mitarbeiterbild aktualisiert');
        return;
      }
