...
assert.match(page, /bg-\[#20c4c1\].*text-\[#062d38\].*hover:bg-\[#45d3d0\]/, 'service add buttons should use the readable turquoise primary style')
...
assert.match(datePicker, /weekday: 'short'/, 'mobile selected date should use a compact weekday')
assert.match(datePicker, /sm:hidden/, 'compact selected date should be limited to mobile')
assert.match(datePicker, /hidden whitespace-nowrap font-bold capitalize text-slate-950 dark:text-white sm:block/, 'desktop should keep the full selected date')
assert.doesNotMatch(datePicker, /block truncate font-bold capitalize/, 'selected date should not be visually truncated')
...
assert.match(button, /bg-\[#20c4c1\]/, 'primary buttons should use the BookaNord turquoise')
assert.match(button, /text-\[#062d38\]/, 'primary button labels should retain strong contrast')
assert.match(button, /hover:bg-\[#45d3d0\]/, 'primary buttons should use the lighter turquoise hover state')
