...
assert.match(servicesPage, /mockLocations/)
assert.match(servicesPage, /location_name:/)
...
const offeringForm = readFileSync('components/OfferingForm.tsx', 'utf8')
assert.match(offeringForm, /isStandaloneBookable/)
assert.match(offeringForm, /Auch einzeln buchbar/)
assert.match(
  offeringForm,
  /nur als Zusatz zu einer\s+Hauptleistung/
)
assert.match(sortableCard, /Auch einzeln buchbar/)
assert.match(sortableCard, /onToggleStandalone/)
assert.match(servicesPage, /handleToggleStandalone/)
assert.match(servicesPage, /isStandaloneBookable/)

const bookingPage = readFileSync('app/book/[slug]/page.tsx', 'utf8')
const legacyBookingPage = readFileSync('app/book/page.tsx', 'utf8')

assert.match(bookingPage, /standaloneOfferings/)
assert.match(bookingPage, /const addonOfferings = offerings\.filter/)
assert.match(bookingPage, /const standaloneServiceOfferings = standaloneOfferings\(offerings\)/)
assert.match(bookingPage, /standaloneServiceOfferings\.map/)
assert.match(bookingPage, /addonOfferings=\{addonOfferings\}/)
assert.match(legacyBookingPage, /standaloneOfferings/)
assert.match(legacyBookingPage, /standaloneServiceOfferings\.map/)
