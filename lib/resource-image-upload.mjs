...
export const OFFERING_IMAGE_BUCKET = 'offering-images'
...
export function getOfferingImageUploadPath({
  organizationId,
  offeringId,
  fileName,
  version = Date.now(),
}) {
  return `${organizationId}/${offeringId}/${version}-${slugifyFileName(fileName)}.webp`
}
...
export async function uploadCompressedOfferingImage({
  supabase,
  file,
  organizationId,
  offeringId,
  ...
  const compressedFile = await compressResourceImage(file)
  const path = getOfferingImageUploadPath({
    organizationId,
    offeringId,
    fileName: compressedFile.name,
  })
  ...
  const { data } = supabase.storage.from(OFFERING_IMAGE_BUCKET).getPublicUrl(path)
  return data.publicUrl
}
